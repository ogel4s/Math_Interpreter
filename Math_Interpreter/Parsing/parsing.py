...
def tokenizer(parsing_list):
    ...
    for ind, tri in enumerate(parsing_list):
        if tri in ['s', 'c', 't', 'a'] and tri.isalpha() and parsing_list[ind + 1].isalpha():
            start = ind
            temp = tri
            ind += 1
            while parsing_list[ind].isalpha():
                temp += parsing_list[ind]
                ind += 1
            
            parsing_list = replacer(parsing_list, temp, start, ind - 1)
            return tokenizer(parsing_list)
